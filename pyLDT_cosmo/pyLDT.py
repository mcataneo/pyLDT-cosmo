...
class matter_pdf:
    ...
    def get_s2_mu(self, R, sigma2, sigma2_fid, s2_mu_fid):
    ...
    def sigma2_gr(self, params, zvec):
    ...
    def sigma2_fr(self, params, zvec, kvec, pk_gr):

        Nz = len(zvec)

        pk_vec = compute_pk.compute_pk_fr_vectorized(params['Omega_m'], params['fR0'], params['n'], zvec, kvec, pk_gr)
    ...
    def sigma2_dgp(self, params, zvec, kvec, pk_gr):

        Nz = len(zvec)

        pk_vec = compute_pk.compute_pk_dgp_vectorized(params['Omega_m'], params['rcH0'], zvec, pk_gr)
        logpk = [scipy.interpolate.CubicSpline(np.log(kvec),np.log(pk_vec[i])) for i in range(Nz)]
    ...
    def get_pdf(self, R, z, tau, s2_mu, sigma2):
